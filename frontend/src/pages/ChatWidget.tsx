...
const ChatWidget: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        const current: Session = stored ? JSON.parse(stored) : (await api.post(`/public/widgets/${widgetId}/sessions`, { visitorId: sessionStorage.getItem('visitor-id') || undefined })).data.data;
  ...
  const send = async () => {
    ...
    try { await api.post(`/public/sessions/${session.sessionId}/messages`, { content: content.trim() }, { headers: { 'X-Visitor-Token': session.token } }); setContent(''); await loadMessages(session); }
  ...
  return <div style={{ minHeight: '100vh', background: '#f4f7f6', padding: 24 }}><Card title="在线咨询" style={{ maxWidth: 680, margin: '0 auto' }} extra={<Button onClick={() => setLeadOpen((value) => !value)}>留下联系方式</Button>}>
    ...
    {leadOpen && <Form form={leadForm} layout="vertical" style={{ marginTop: 16 }}><Form.Item name="phone" label="手机号"><Input /></Form.Item><Form.Item name="email" label="邮箱"><Input /></Form.Item><Button type="primary" onClick={() => void submitLead()}>提交联系方式</Button></Form>}
  </Card></div>;
};
