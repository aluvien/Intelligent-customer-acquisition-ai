...
const mockChannelData = [
  { name: '抖音', value: 45, color: '#0E7C6B' },
  { name: '快手', value: 30, color: '#0A5A4F' },
  { name: '视频号', value: 20, color: '#12A086' },
  { name: '小红书', value: 5, color: '#7FD1C0' },
];
...
const mockSatisfactionData = [
  { name: '非常满意', value: 45, color: '#12A086' },
  { name: '满意', value: 35, color: '#0E7C6B' },
  { name: '一般', value: 15, color: '#D97706' },
  { name: '不满意', value: 5, color: '#E5484D' },
];
...
const Dashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const [dashboardRes, conversationsRes, channelsRes] = await Promise.all([
        ...
        api.get('/conversations/data').then(res => res.data),
        api.get('/channels/data').then(res => res.data),
      ]);
      ...
      if (channelsRes.success) {
        ...
        const channelData = channelsRes.data.channels.map((channel: any) => ({
          ...
          color: channel.status === 'connected' ? '#12A086' : 
                 channel.status === 'warning' ? '#D97706' : '#E5484D'
        }));
  ...
  const loadRealtimeData = async () => {
    try {
      // 调用真实API获取实时数据
      const result = await api.get('/analytics/realtime').then(res => res.data);
  ...
  return (
    <div style={{ padding: '4px 0' }}>
          ...
          <Timeline>
            {mockSystemAlerts.map((alert, index) => (
              ...
              >
                ...
                <span style={{ color: BUSINESS_THEME.textSecondary, marginLeft: '8px' }}>({alert.time})</span>
              </Timeline.Item>
            ))}
          </Timeline>
        }
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card style={{ borderColor: BUSINESS_THEME.border }}>
            ...
            <div style={{ marginTop: '8px' }}>
              <span style={{ color: '#12A086' }}>
                <RiseOutlined /> +12.5%
              </span>
              <span style={{ color: BUSINESS_THEME.textSecondary, marginLeft: '8px' }}>较昨日</span>
            </div>
          </Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card style={{ borderColor: BUSINESS_THEME.border }}>
            ...
            <div style={{ marginTop: '8px' }}>
              ...
              <span style={{ color: BUSINESS_THEME.textSecondary, marginLeft: '8px' }}>今日新增</span>
            </div>
          </Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card style={{ borderColor: BUSINESS_THEME.border }}>
            <Statistic
              ...
              valueStyle={{ color: stats.systemLoad > 80 ? '#E5484D' : '#12A086' }}
              suffix="%"
            />
            <div style={{ marginTop: '8px' }}>
              <Progress 
                ...
                size="small" 
                showInfo={false}
                strokeColor={stats.systemLoad > 80 ? '#E5484D' : '#12A086'}
              />
            </div>
          </Card>
        </Col>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
        <Col xs={24} lg={16}>
          <Card style={{ borderColor: BUSINESS_THEME.border }} title="对话趋势分析" extra={<Button type="link">详细分析</Button>}>
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={mockTrendData}>
                ...
                <Area 
                  ...
                  stackId="1"
                  stroke={BUSINESS_THEME.primary} 
                  fill={BUSINESS_THEME.primary}
                  fillOpacity={0.6}
                  name="对话数"
                />
                <Area 
        ...
        <Col xs={24} lg={8}>
          <Card style={{ borderColor: BUSINESS_THEME.border }} title="渠道分布" extra={<Button type="link">查看详情</Button>}>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
        <Col xs={24} lg={12}>
          <Card style={{ borderColor: BUSINESS_THEME.border }} title="渠道绩效对比" extra={<Button type="link">详细分析</Button>}>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={mockChannelPerformance}>
                ...
                <YAxis />
                <RechartsTooltip />
                <Bar dataKey="messages" fill={BUSINESS_THEME.primary} name="消息数" />
                <Bar dataKey="leads" fill="#12A086" name="线索数" />
              </BarChart>
            </ResponsiveContainer>
          </Card>
        </Col>
        <Col xs={24} lg={12}>
          <Card style={{ borderColor: BUSINESS_THEME.border }} title="客户满意度分布" extra={<Button type="link">查看详情</Button>}>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
        <Col xs={24} lg={12}>
          <Card style={{ borderColor: BUSINESS_THEME.border }} title="客服绩效排行" extra={<Button type="link">详细分析</Button>}>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={mockAgentPerformance} layout="horizontal">
                ...
                <RechartsTooltip />
                <Bar dataKey="conversations" fill={BUSINESS_THEME.primary} name="对话数" />
              </BarChart>
            </ResponsiveContainer>
          </Card>
        </Col>
        <Col xs={24} lg={12}>
          <Card style={{ borderColor: BUSINESS_THEME.border }} title="系统性能雷达图" extra={<Button type="link">查看详情</Button>}>
            <ResponsiveContainer width="100%" height={300}>
              <RadarChart data={mockRadarData}>
                <PolarGrid />
                <PolarAngleAxis dataKey="subject" />
                <PolarRadiusAxis angle={30} domain={[0, 150]} />
                <Radar name="当前" dataKey="A" stroke={BUSINESS_THEME.primary} fill={BUSINESS_THEME.primary} fillOpacity={0.6} />
                <Radar name="目标" dataKey="B" stroke="#12A086" fill="#12A086" fillOpacity={0.6} />
                <RechartsTooltip />
              </RadarChart>
            </ResponsiveContainer>
          </Card>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={16}>
          <Card
            style={{ borderColor: BUSINESS_THEME.border }}
            title="最近对话" 
            extra={
              <Space>
        ...
        <Col xs={24} lg={8}>
          <Card style={{ borderColor: BUSINESS_THEME.border }} title="系统状态" extra={<Button type="link">监控中心</Button>}>
            <List
              dataSource={[
