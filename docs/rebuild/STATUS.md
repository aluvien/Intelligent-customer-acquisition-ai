...
| T2 数据库/认证 | passed locally | PostgreSQL 全量迁移（含全局大小写不敏感用户名唯一性）、真实用户/租户、原子刷新会话轮换、JWT 类型/租户状态校验、Redis WS ticket 和加密凭据已在临时 PostgreSQL 集成环境验证 |
| T3 业务任务链路 | passed locally | 标准事件、消息去重、事务内持久化 job、租约/重试、发送状态和 Worker 已实现；仍需故障注入矩阵 |
...
| T9 发布验收 | in-progress | 已加入 `.github/workflows/ci.yml`；Node/前端构建、Compose 配置、PostgreSQL/Redis 集成和依赖审计已执行；Go 编译和干净 Docker 镜像需由 GitHub CI 或具备工具链的环境验证，真实 AI/平台仍保持 blocked |
