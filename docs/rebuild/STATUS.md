...
| T2 数据库/认证 | passed locally | PostgreSQL 迁移、真实用户/租户、可撤销刷新会话、JWT 类型/租户状态校验、Redis WS ticket 和加密凭据已通过本机集成链路 |
| T3 业务任务链路 | passed locally | 标准事件、消息去重、持久化 job、租约/重试、发送状态和 Worker 已通过本机人工回复投递验收；仍需故障注入矩阵 |
