...
| T9 发布验收 | in-progress | 已加入 `.github/workflows/ci.yml`；Node/前端构建、前后端测试、Compose 配置、全量 PostgreSQL 迁移/约束、Redis/HTTP 集成和后端依赖审计已执行；Go 编译和干净 Docker 镜像需由 GitHub CI 或具备工具链的环境验证，真实 AI/平台仍保持 blocked |
