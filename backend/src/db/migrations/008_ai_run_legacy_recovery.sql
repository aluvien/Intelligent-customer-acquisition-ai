...
ALTER TABLE ai_runs
  ADD COLUMN IF NOT EXISTS knowledge_snapshot_state TEXT NOT NULL DEFAULT 'pending';
...
UPDATE ai_runs
 ...
 WHERE created_at < (
   SELECT applied_at FROM schema_migrations WHERE name = '007_ai_provider_recovery.sql'
 );
